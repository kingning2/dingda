...
class XiaohongshuCrawler(BrowserCrawler):
    ...
    async def search(self, ctx: CrawlContext, query: str) -> CrawlResult:
        ...
        try:
            ...
            from crawler.core.live import LIST_DWELL_S, dwell_for_viewer
...
def _enrich_with_ocr(item: Any) -> Any:
    ...
    from crawler.ocr import ocr_image_urls
...
async def _enrich_with_page_ocr(item: Any, page: Any) -> Any:
    ...
    from crawler.core.types import CrawlItem
    from crawler.ocr import ocr_image_bytes
...
async def _drain_comments(pending: list[Any], captured: list[dict[str, Any]]) -> None:
    """读取 comment/page 响应。"""
    from crawler.extraction.config import dig_first, path_list, section
    from crawler.sources.xiaohongshu.extractor import EXTRACT
...
def _detail_params(ctx: CrawlContext) -> dict[str, str] | None:
    from crawler.sources.xiaohongshu.extractor import EXTRACT
    from crawler.extraction.config import section
