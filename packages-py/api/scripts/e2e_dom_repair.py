...
from crawler.extraction.config import reload_extract_json  # noqa: E402
from crawler.extraction.repair.orchestrator import repair_detail_dom  # noqa: E402
from crawler.extraction.repair.types import RepairResult  # noqa: E402
from crawler.registry import cookies_for, create_crawler  # noqa: E402
...
def _raw_view(platform: str, raw: Any, page: Page | None = None) -> Page:
    ...
    if platform == "xianyu":
        from crawler.sources.xianyu.crawler import _RawPageView

        return _RawPageView(raw)
    from crawler.sources.xiaohongshu.crawler import _RawPageView as XhsView
...
def _adapter(platform: str) -> Any:
    if platform == "xianyu":
        from crawler.sources.xianyu.crawler import _XIANYU_ADAPTER

        return _XIANYU_ADAPTER
    from crawler.sources.xiaohongshu.crawler import _XHS_ADAPTER
...
async def _pass_platform_risk(platform: str, page: Page) -> None:
    ...
    """
    if platform == "xianyu":
        from crawler.sources.xianyu.crawler import _pass_risk_or_raise

        await _pass_risk_or_raise(page, where="e2e")
        return
    from crawler.sources.xiaohongshu.crawler import _XHS_RISK
...
async def _crawler_detail(
    ...
    if platform == "xianyu":
        ...
        with patch(
            "crawler.sources.xianyu.crawler.mtop_call",
