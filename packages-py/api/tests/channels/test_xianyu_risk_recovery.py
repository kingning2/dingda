...
from channels.xianyu.channel import XianyuQrChannel, XianyuLoginRuntime
...
def test_qr_timeout_triggers_risk_recovery() -> None:
    ...
    with (
        patch(
            "channels.xianyu.channel.api.open_login_page",
        ) as open_login,
        patch(
            "channels.xianyu.channel.api.capture_qr",
            return_value=("qrpng", "https://qr"),
        ),
        patch(
            "channels.xianyu.channel.api.has_login_completed",
            return_value=False,
        ),
        patch(
            "channels.xianyu.channel.api.has_all_login_cookies",
            return_value=False,
        ),
        patch(
            "channels.xianyu.channel.api.has_scanned_cookies",
            return_value=False,
        ),
        patch(
            "channels.xianyu.channel.api.cookie_map",
            return_value={"unb": "123"},
        ),
        patch(
            "channels.xianyu.channel.renew",
            return_value=(
                True,
                "自动滑块通过",
        ...
        patch("channels.xianyu.channel.time.sleep"),
        patch("channels.xianyu.channel.time.monotonic") as monotonic_mock,
    ):
