...
from channels.xianyu.item import delete, extract, items
...
def test_items() -> None:
    ...
    with (
        patch(
            "channels.xianyu.item.Session.from_cookie_header",
            return_value=session,
        ),
        patch("channels.xianyu.item.mtop_call", return_value=mtop_raw),
    ):
...
def test_delete() -> None:
    ...
    with (
        ...
        patch(
            "channels.xianyu.item.mtop_call",
            return_value={"ret": ["SUCCESS::ok"]},
        ),
        patch("channels.xianyu.item.acquire", return_value=nullcontext()),
        patch("channels.xianyu.item.hold", return_value=nullcontext()),
    ):
