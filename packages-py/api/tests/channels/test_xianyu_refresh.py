...
from channels.xianyu.refresh import _profile_from_nav, token
...
class TestRefreshToken:
    ...
    def test_refresh_success(self) -> None:
        ...
        with (
            patch(
                "channels.xianyu.refresh.Session.from_cookie_header",
                return_value=session,
            ),
            patch("channels.xianyu.refresh._ping_login"),
        ):
    ...
    def test_fallback_to_browser(self) -> None:
        ...
        with (
            ...
            patch(
                "channels.xianyu.refresh._ping_login",
                side_effect=ping_side_effect,
            ),
            patch(
                "channels.xianyu.refresh.refresh",
                return_value={
                    "unb": "123",
                    "_m_h5_tk": "from_camoufox",
                    "cookie2": "c",
...
def test_profile_calls_page_nav() -> None:
    ...
    with (
        ...
        patch("channels.xianyu.refresh.mtop_call", return_value=raw) as mtop,
    ):
        from channels.xianyu.refresh import profile
