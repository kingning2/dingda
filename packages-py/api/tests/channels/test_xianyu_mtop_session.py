...
from channels.xianyu.mtop import _classify_error, call as mtop_call
...
def test_session_from_cookie_header(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        "channels.xianyu.session._device_cache_path",
        lambda: tmp_path / "device.json",
    )
    with patch("channels.xianyu.session.generate_device_id", return_value="dev-1"):
...
def test_mtop_call_success() -> None:
    ...
    with patch("channels.xianyu.mtop.generate_sign", return_value="sig"):
