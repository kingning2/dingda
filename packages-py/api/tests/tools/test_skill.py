...
_SKILLS_DIR = Path(skill_mod.__file__).resolve().parent / "skills"
_UTF8_ENV = {**os.environ, "PYTHONIOENCODING": "utf-8"}


def test_render_skills_includes_multi_round_compare() -> None:
...
def test_compose_skills_prompt_stages_cwd_alias(tmp_path: Path, monkeypatch) -> None:
    ...
    prompt = compose_skills_prompt("python", cwd=tmp_path)
...
def test_compose_skills_prompt_runs_headroom(tmp_path: Path, monkeypatch) -> None:
    ...
    # 用 setitem 注入替身：pytest 结束后自动还原，不会把假 headroom
    # 泄漏给同进程后续用例（否则真实压缩路径被顶掉）。
