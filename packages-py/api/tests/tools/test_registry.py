...
from crawler.core.types import CrawlItem, CrawlResult
from tools.compare import CompareOutput
from tools.product import ProductOutput
from tools.search import SearchOutput
...
def test_call_product_success_mocked() -> None:
    ...
    async def _run() -> None:
        with (
            patch(
                "tools.product.get_browser_manager",
            ) as get_manager,
            patch("tools.product.create_crawler") as create,
        ):
            manager = AsyncMock()
...
def test_call_search_ali1688_mocked() -> None:
    ...
    async def _run() -> None:
        with patch("tools.search.create_api_crawler") as create:
            crawler = AsyncMock()
...
def test_call_compare_mocked() -> None:
    from crawler.sources.ali1688.compare import CompareResult, CompareSource
    ...
    async def _run() -> None:
        with patch(
            "tools.compare.compare_products",
