...
from crawler.extraction import fingerprint as fp
...
def test_xhs_needs_repair_covers_missing_author() -> None:
    ...
    from crawler.sources.xiaohongshu.crawler import _needs_repair
...
def test_repair_detail_dom_relocates_via_fingerprint_and_persists(tmp_path: Path) -> None:
    ...
    async def _run() -> RepairResult:
        with patch("crawler.extraction.fingerprint._STORE", tmp_path / "fp.json"):
            fp.save_fingerprint(
                "xianyu",
                "detail_dom",
                "price",
