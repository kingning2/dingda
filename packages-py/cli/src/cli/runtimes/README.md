...
**不再往各 CLI 注入 MCP。** 工具由 [`../skill.py`](../skill.py) 渲染成
...
命令形态是 `"<python>" -m tools.cli <tool> --flags`，stdout 纯 JSON。
...
`python -m tools.validate_cli`；比工具总线白名单更窄，也不会带上别的工具。
