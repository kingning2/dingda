...
def test_qr_check_success_persists_account(tmp_path) -> None:
    ...
    with patch(
        ...
        "src.domains.account.persist.enrich_account_profile_async",
    ):
    ...
    assert item["display_name"] == "闲鱼账号 123"
...
def test_account_connect_and_disconnect(account_client: TestClient) -> None:
    ...
    with patch(
        "src.domains.account.service.enrich_account_profile",
        return_value=False,
    ):
...
def test_account_delete_not_found(account_client: TestClient) -> None:
