...
from src.adapters.channel.camoufox_browser import headless_page
...
class XiaohongshuQrChannel(QrLoginChannel):
    ...
    def _maybe_publish_login(
        ...
        try:
            ...
            logger.info("小红书扫码登录成功: %s", account_id)
    ...
    def _run(self, runtime: XiaohongshuLoginRuntime, timeout: int) -> None:
        ...
        try:
            ...
            with headless_page() as page:
                ...
                runtime.ready.set()
                logger.info("小红书二维码就绪 qr_id=%s", qr_id)

                deadline = time.monotonic() + timeout
                while time.monotonic() < deadline:
                    if login_complete.is_set() and runtime.cookie:
                        break
