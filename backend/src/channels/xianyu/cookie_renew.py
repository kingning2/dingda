"""闲鱼扫码/登录风控恢复：无头 Camoufox 自动滑块 → 有头 Camoufox 人工兜底。"""
...
from src.adapters.channel.camoufox_browser import (
    close_camoufox_renew_context,
    cookies_to_playwright,
    launch_camoufox_renew_context,
)
from src.adapters.channel.playwright_chrome import to_serializable_cookies
from src.adapters.channel.slider import (
    auto_slider_enabled,
    clear_risk_cookies,
    has_x5sec,
    try_solve_slider,
...
logger = logging.getLogger("dingda.channel.xianyu.cookie_renew")
...
async def _run_camoufox_session(
    *,
    ...
    cm = None
    try:
        cm, context = await launch_camoufox_renew_context(user_data_dir, headless=headless)
        ...
        if cm is not None:
            await close_camoufox_renew_context(cm)
...
async def _recover_async(
    ...
    *,
    ...
    prepared = cookies_to_playwright(cookies or {})
    if not prepared and not punish_url:
        prepared = []
...
def recover_xianyu_login(
    ...
    *,
    ...
    """扫码失败或触发风控时：无头 Camoufox 自动滑块 → 有头 Camoufox 人工兜底。"""
    return asyncio.run(
        _recover_async(
            cookies,
            account_id=account_id,
