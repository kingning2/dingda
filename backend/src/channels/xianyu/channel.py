...
from src.adapters.channel.qr_terminal import decode_qr_text_from_png_base64
...
from src.channels.xianyu.cookie_renew import recover_xianyu_login
...
class XianyuQrChannel(QrLoginChannel):
    ...
    def start_login(self, *, timeout: int = 120) -> XianyuLoginRuntime:
        ...
        thread = threading.Thread(target=self._run, args=(runtime, timeout), daemon=True)
        thread.start()
        return runtime
    ...
    def snapshot(self, runtime: XianyuLoginRuntime) -> LoginSnapshot:
        with runtime.lock:
            if runtime.cookies:
                ...
                display_name = tracknick or f"闲鱼账号 {unb}"
    ...
    def _try_risk_recovery(
        ...
        *,
        ...
        ok, detail, cookies = recover_xianyu_login(
            partial_cookies,
