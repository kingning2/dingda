...
use crate::runtime::resolution::{canonicalize_path, is_executable_file};
use crate::runtime::types::{
    ExecutableSource, ManagedDownloadSpec, ResolvedExecutable, RuntimeDefinition,
};
...
impl RuntimeDefinition {
    ...
    fn managed_binary_file_name(&self) -> String {
        #[cfg(windows)]
        {
...
async fn download_with_spec(
    ...
    log_base("开始下载", Some(&format!("{} {}", definition.id, url)));
...
async fn fetch_text(url: &str) -> Result<String, String> {
    ...
    if !response.status().is_success() {
        return Err(format!("读取版本失败：HTTP {}", response.status()));
    }
    response
        .text()
        .await
        .map_err(|e| format!("读取版本内容失败: {e}"))
}
...
fn log_base(message: &str, detail: Option<&str>) {
    match detail {
        Some(d) => eprintln!("[runtime/defs/base] {message}: {d}"),
        None => eprintln!("[runtime/defs/base] {message}"),
    }
}

#[cfg(test)]
mod tests {
    use crate::runtime::defs::{CLAUDE, CODEX, OPENCODE};
