...
| [frontend-coding](frontend-coding/SKILL.md) | 前端 **通用规范模板**（与项目无关，可直接复制到其他项目）：文件粒度、同类归文件夹、注释、命名、字段容错 |
...
- 新业务目录：`packages-py/{agent,crawler,browser,tools}/src/<pkg>/`（uv 工作区）。
...
- 写前端必须按 `frontend-coding` 的示例形状落笔：**一个功能块一个文件**、
  同类归文件夹（禁 `utils/` `helpers/` `services/` 这类垃圾桶名）、文件头写「一行总述 + 职责：」。
  **那份 Skill 是通用模板、不含本项目路径**，叮答特有的落点（源码根、共享模块、
  检查命令）写在 `.cursor/rules/frontend-coding.mdc`。
