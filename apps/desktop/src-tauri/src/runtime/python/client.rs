...
/// 本地 Python Sidecar 共享内存客户端；段文件由 [`Self::new`] 创建，
/// 路径经 `--shm` 参数传给 Python 进程。
#[derive(Clone)]
...
impl SidecarClient {
    /// 创建共享内存段并构建客户端。
    pub fn new(shm_path: &Path) -> Result<Self, SidecarClientError> {
        let transport = ShmTransport::create(shm_path).map_err(SidecarClientError::from)?;
        Ok(Self {
            transport,
