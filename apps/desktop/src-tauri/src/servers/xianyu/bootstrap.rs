...
//!
//! 账号 CRUD / 扫码登录等两站共用 Handle 由 `servers::core::bootstrap` 无条件注册；
...
use crate::runtime::python::wss_bridge::connect_channel;
use crate::runtime::python::PythonWssBridge;
use crate::servers::protocol::dispatcher::ChannelDispatcher;
...
pub fn register_business(
    ...
    let engine = Arc::new(crate::servers::xianyu::monitor::MonitorEngine {
    ...
    let task_manager = app
        ...
        .tasks()
        .clone();
    let scheduler = Arc::new(crate::servers::xianyu::monitor::MonitorScheduler::new(
        engine.clone(),
        1,
        task_manager,
    ));
