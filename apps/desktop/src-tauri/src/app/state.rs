...
/// - 持有进程内事件总线
/// - 持有 Runtime 总协调器（控制层；App 观测走 `core::manager::app`）
///
/// 作者：coisini
/// 创建时间：2026-07-16
#[derive(Clone)]
pub struct AppState {
    ...
    /// 进程内事件总线（runtime.* 事件经 `core::infra::event::BusToTauri` 转发到前端）。
