...
//!
//! 壳层组合：`InMemoryAccountStore` → `crate::servers::domain::account::AccountService`（校验 + 编排）。
...
pub async fn account_probe_login(
    ...
    let ok = if platform == "ali1688" {
        ...
        match crate::runtime::python::routes::xianyu_user_profile::call(
            app_state.lifecycle.client(),
            crate::runtime::python::routes::xianyu_user_profile::UserProfileRequest {
