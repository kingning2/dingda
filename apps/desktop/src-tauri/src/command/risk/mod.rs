...
//!
//! 壳层组合：`InMemoryRiskStore` → `crate::servers::domain::risk::RiskService`。

use crate::command::IpcResponse;
use crate::servers::domain::risk::{RiskConfig, RiskLogPage, RiskLogQuery, RiskService};
use crate::servers::xianyu::persist::InMemoryRiskStore;
...
) -> crate::contracts::DingDaResult<IpcResponse<crate::servers::domain::risk::RiskTodaySuccessRate>>
{
    let service = RiskService::new(state.store.as_ref());
