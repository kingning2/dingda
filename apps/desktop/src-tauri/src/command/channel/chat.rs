...
use crate::runtime::python::routes::xianyu_message_headinfo::{self, MessageHeadinfoRequest};
...
pub async fn channel_fetch_history(
    ...
    let my_unb = repo
        ...
        .and_then(|account| {
            let cookie_list =
                crate::servers::shared::cookies::parse_credential(&account.credential);
            crate::servers::shared::cookies::my_id(&cookie_list)
        });
...
pub async fn channel_product_headinfo(
    ...
    let cookie_str = crate::servers::shared::cookies::cookies_to_string(
        &crate::servers::shared::cookies::parse_credential(&account.credential),
    );
