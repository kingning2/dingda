...
use crate::command::AccountHandle;
use crate::command::IpcResponse;
use crate::runtime::python::routes::{
    xianyu_item_detail::{self, ItemDetailRequest, PlatformItemDetailDto},
    xianyu_seller_items::{self, SellerItemsRequest},
};
use crate::servers::domain::account::{AccountService, AccountStore, AccountUpdate, XianyuAccount};
use crate::servers::domain::item::{Item, ItemQuery, ItemService};
use crate::servers::xianyu::persist::InMemoryItemStore;
...
pub async fn item_sync(
    ...
    for account in targets {
        let cookie_header =
            crate::servers::shared::cookies::credential_to_cookie_header(&account.cookie);
        let user_id = {
            let from_account = account.extract_unb();
            if !from_account.is_empty() {
                from_account
            } else {
                crate::servers::shared::cookies::my_id(
                    &crate::servers::shared::cookies::parse_credential(&cookie_header),
                )
                .unwrap_or_default()
            }
        };
...
pub async fn item_detail_fetch(
    ...
    let response = xianyu_item_detail::call(
        state.lifecycle.client(),
        ItemDetailRequest {
            cookie: crate::servers::shared::cookies::credential_to_cookie_header(&account.cookie),
