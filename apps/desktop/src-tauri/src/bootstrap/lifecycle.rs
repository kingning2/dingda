...
//!
//! 只记录，不含任何运行控制（控制层在 `crate::infrastructure::runtime::supervisor`）；
...
/// 平台就绪后后台启动账号探活（闲鱼轮换 + 其余探活）。
pub fn spawn_startup_account_probe(app: &tauri::AppHandle) {
    crate::bootstrap::startup_probe::spawn(app);
}
