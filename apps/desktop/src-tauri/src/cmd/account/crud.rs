...
//!
//! 壳层组合：`InMemoryAccountStore` → `crate::core::domain::account::AccountService`（校验 + 编排）。
...
pub async fn account_probe_login(
    ...
    let ok = if platform == "ali1688" {
        ...
        if cookies.is_empty() {
            ...
            match crate::core::manager::python::routes::channel_login_probe::call(
                sidecar,
                sidecar_request,
            )
            .await
        ...
        match crate::core::manager::python::routes::xianyu_user_profile::call(
            app_state.lifecycle.client(),
            crate::core::manager::python::routes::xianyu_user_profile::UserProfileRequest {
