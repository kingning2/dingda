//! Python Runtime — 仅 Sidecar 生命周期（进程 / 传输 / 健康 / 日志）。
//!
//! 业务路径绑定在 [`crate::infrastructure::channel::sidecar`] 与 [`crate::infrastructure::runtime::agent::sidecar`]。
...
impl PythonRuntime {
    ...
    pub async fn sync_snapshot(&self) -> Option<PythonSidecarSnapshot> {
        ...
        match runtime_status::fetch(self.ipc.client()).await {
            Ok(snap) => {
