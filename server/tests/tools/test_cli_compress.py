...
def test_cli_main_applies_compress_tool_payload(capsys, monkeypatch) -> None:
    ...
    with patch("src.tools.registry.list_tools", return_value=[spec]):
        with patch("src.tools.registry.get_tool", return_value=spec):
            with patch(
                "src.tools.registry.call_tool",
                return_value=payload,
            ):
                with patch(
                    "src.agent.core.compress.compress_tool_payload",
