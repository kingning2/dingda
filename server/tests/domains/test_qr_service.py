...
def test_second_start_cancels_inflight_session() -> None:
    waiting = LoginSnapshot(status=LoginStatus.WAITING, qr_base64="ZmFrZQ==")
    first_channel, first_runtime = _mock_channel(waiting)
    second_channel, _second_runtime = _mock_channel(waiting)
    ...
    with patch(
        ...
        service = ChannelQrService()
        first = service.start(QrStartRequest(platform="xiaohongshu"))
        second = service.start(QrStartRequest(platform="xiaohongshu"))

    first_runtime.cancel.set.assert_called()
    with pytest.raises(AppError) as exc:
        service.check(first.session_id)  # type: ignore[arg-type]
