...
from src.channels.xianyu.message import chats, parse_session_row, send
...
def test_chats_baseline() -> None:
    ...
    async def _run() -> None:
        with (
            patch(
                "src.channels.xianyu.message.Session.from_cookie_header",
                return_value=session,
            ),
            patch(
                "src.channels.xianyu.message.mtop_call",
...
def test_send_text() -> None:
    ...
    async def _run() -> None:
        with (
            ...
            patch(
                "src.channels.xianyu.message.get_access_token",
                return_value="tok",
            ),
            patch("src.channels.xianyu.message.connect") as connect_cm,
            patch(
                "src.channels.xianyu.message.register",
                new_callable=AsyncMock,
            ),
            patch(
                "src.channels.xianyu.message.heartbeat_loop",
                new_callable=AsyncMock,
            ),
            patch(
                "src.channels.xianyu.message.send_text",
                new_callable=AsyncMock,
                return_value="mid-1",
            ),
            patch("src.channels.xianyu.message.asyncio.sleep", new_callable=AsyncMock),
            patch("src.channels.xianyu.message.acquire", return_value=nullcontext()),
            patch("src.channels.xianyu.message.hold", return_value=nullcontext()),
        ):
            ws = MagicMock()
