...
from src.cli.repair import propose as repair_spawn
from src.crawler.extraction.repair.types import DomSnapshot
...
def test_session_log_reports_tool_surface() -> None:
    """日志要如实写：注入了哪些工具、拼不拼父前言。"""
    from src.cli.base import _mcp_config_line

    narrow = _mcp_config_line(
        "codex-mcp", tools=["validate_selectors"], uses_system_prompt=False
    )
    assert "validate_selectors" in narrow
    assert "仅本次 prompt" in narrow
    assert "system.md" not in narrow
    assert "product" not in narrow

    assert "未注入" in _mcp_config_line("none", uses_system_prompt=False)
