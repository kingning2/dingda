...
- `server/scripts/probe_xianyu_detail_dom.py` — 过滑块 + 扫 DOM 候选
- `server/scripts/probe_xianyu_detail_struct.py` — **拉主区 HTML/DOM 树并分析节点**
- 产物目录：`server/tmp/xianyu_detail_dom_probe/`（`struct.json` = 结构分析结果）
