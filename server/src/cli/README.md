...
外部 CLI Agent Runtime：在 Python Server 内启动 Codex / Claude / OpenCode，注入 dingda-mcp，流式产出事件。
...
`CliRuntime`（插座）：一次会话的生命周期 —— 解析可执行文件 → 注入 MCP → 拼 prompt → spawn →
流式解析 → 会话日志。插头只填 `id` / `name` / `binary` / `mcp_mode` / `stream_format` 并实现 `build_args`。

`run(..., role=...)` 按**角色**取三条策略（提示词怎么拼 / MCP 给不给 / cwd 落哪），
...
- [inject/](inject/README.md) — 注入 dingda-mcp
