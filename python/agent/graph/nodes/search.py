...
from agent.llm.tool_loop import run_tool_loop
from skills.market_research.skill import RESEARCH_TOOL_NAMES, SKILL_ID, research_tools
from skills.registry import skill_prompt_for_ai
from tools.alibaba import search_alibaba
from tools.knowledge import retrieve_knowledge
from tools.web.bindings import format_web_context, make_web_fetch_tool, sources_from_tool_payloads
...
def _research_system() -> str:
    return "\n".join(
        [
            skill_prompt_for_ai(SKILL_ID, only=RESEARCH_TOOL_NAMES),
            "",
...
def web_research_node(state: GraphState, ctx: GraphContext) -> dict[str, Any]:
    ...
    def _on_tool(name: str, args: dict[str, Any], payload: str) -> None:
        if name in ("web_fetch", "web_scrape"):
...
async def crawl_node(state: GraphState, ctx: GraphContext) -> dict[str, Any]:
    ...
    if not (account_id and isinstance(cookies, list) and keywords):
        return {}
    ...
    return {
        "xianyu_items": xianyu_all or (state.get("xianyu_items") or []),
        "alibaba_items": alibaba_all or (state.get("alibaba_items") or []),
    }
