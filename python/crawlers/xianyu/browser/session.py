"""闲鱼 Camoufox/Chromium 会话辅助 — Cookie 注入、profile 目录、登录/风控 URL 判定。

搜索、续期与扫码登录在启动浏览器前复用本模块准备目录与 Cookie。"""
...
def profile_dir(account_id: str) -> Path:
    configured = os.getenv("DINGDA_XIANYU_BROWSER_PROFILE", "").strip()
    if not configured:
        configured = os.getenv("DINGDA_XIANYU_SEARCH_PROFILE", "").strip()
    if configured:
        return Path(configured)
    return Path.cwd() / "browser_data" / f"user_xianyu_{safe_account_dir(account_id)}"
...
def playwright_cookie(raw: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not name or not value:
        return None
    domain = str(raw.get("domain") or "").strip() or ".goofish.com"
