...
func NewBrowser(headless bool, options ...Option) *headless_browser.Browser {
	...
	// 只用内置浏览器，没有别的来源。二进制由 DingDa 应用（Rust 插件系统）下载，
	// 这里显式解析路径并传给 go-rod，否则 rod 会自行下载一个默认 Chromium：
	// 它不是内置浏览器，也不认识下面这些 flag（未知 flag 被静默忽略，日志照样打印
	// "fingerprint enabled"），属于无声降级。宁可不启动，也不启动一个不对的浏览器。
	binPath, err := ResolveBrowser()
	if err != nil {
