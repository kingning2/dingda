...
from dingda_sidecar.tools.registry import bind_skill_tools, format_tools_for_ai, tools_for_skill
...
def make_skill_tools() -> list[BaseTool]:
    ...
    名称 = skill id；description / args_schema 来自 SkillSpec。
    公用原子能力请用 ``tools.registry``，不要把长流程做成 skill。
    """
    ensure_builtin_skills()
    out: list[BaseTool] = []
...
def ensure_builtin_skills() -> None:
    """加载 builtin skill 规格（当前无比价类 skill；公用能力在 ``tools.registry``）。"""
