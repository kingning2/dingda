...
def _looks_logged_in(
    ...
    *,
    ...
    home_host: str = "goofish.com",
) -> bool:
    """用 Cookie 与当前 URL 判断是否仍保持登录。"""
    has_login = any(c.get("name") == login_cookie for c in cookies)
    url = page_url.lower()
    if platform_name == "xiaohongshu":
        blocked = "/login" in url or "captcha" in url
        has_a1 = any(c.get("name") == "a1" for c in cookies)
        return has_login and has_a1 and not blocked
    ...
    if has_login and has_tk and has_x5sec(cookies):
        return True
    return has_login and has_tk and not blocked


def _session_cookie_ready(
    exported: list[dict[str, Any]],
    *,
    platform_name: str,
    login_cookie_name: str,
) -> bool:
    has_login = _has_cookie(exported, login_cookie_name)
    if platform_name == "xiaohongshu":
        return has_login and _has_cookie(exported, "a1")
    return has_login and _has_cookie(exported, "_m_h5_tk")
...
async def _export_after_slider(
    *,
    ...
    for attempt in range(1, 4):
        ...
        logger.info(
            "回访后 Cookie account=%s exported=%s login=%s ready=%s x5sec=%s names=%s url=%s",
            account_id,
            len(exported),
            has_login,
            ready,
        ...
        if has_login and platform_name != "xiaohongshu":
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if _looks_logged_in(
                ...
                if platform_name != "xiaohongshu" and (
                    ("punish" in url.lower() or "captcha" in url.lower())
                    and not has_x5sec(exported)
                ):
                    last_detail = "仍在验证码页，请完成滑块"
                    await page.wait_for_timeout(1500)
                    continue
                if config.cookie_domain_keyword not in url.lower():
                    with contextlib.suppress(Exception):
                        await page.goto(
                            config.home_url,
                            wait_until="domcontentloaded",
