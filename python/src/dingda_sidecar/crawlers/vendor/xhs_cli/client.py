...
class XhsClient:
    ...
    def start(self):
        ...
        if self._executable_path:
            # Camoufox 包对自定义 exe 必须显式 ff_version，否则去查自身 official/stable。
            kwargs["executable_path"] = self._executable_path
            kwargs["ff_version"] = self._ff_version or 152
            logger.info("using custom camoufox: %s", self._executable_path)
