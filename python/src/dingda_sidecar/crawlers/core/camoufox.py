...
def resolve_camoufox_executable_with_version() -> tuple[str | None, int]:
    """返回 (系统 Camoufox exe 路径, Firefox 主版本)；未安装时 (None, 0)。

    供调用方在外部浏览器库（如 vendored xhs_cli）里复用系统 Camoufox。
    """
    exe = resolve_camoufox_executable()
    if exe is None:
        return None, 0
    return str(exe), _infer_ff_version(exe)
...
async def browser_page_session(
    *,
    ...
    """启动系统浏览器会话（Camoufox 优先，回退 Chromium），注入 Cookie，yield Page。

    供各渠道搜索/抓取适配器复用；``cookies`` 为 Playwright ``add_cookies`` 格式。
    """
    user_data_dir.mkdir(parents=True, exist_ok=True)
