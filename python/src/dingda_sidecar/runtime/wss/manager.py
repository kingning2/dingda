...
from dingda_sidecar.crawlers.goofish.login.cookie_renew import renew_cookies
...
# from runtimes.wss.auto_reply import maybe_auto_reply  # 买家自动回复入口暂关，见 on_event

logger = logging.getLogger("dingda.runtimes.wss")

EventCallback = Callable[[dict[str, Any]], Awaitable[None] | None]
WS_EVENT_METHOD = "ws.event"
...
class WssManager:
    """进程内 WSS 连接注册表 — Rust 启停长连接，入站经 pipe Event 推送。"""
        ...
        async with self._lock:
            ...
            async def on_event(event: dict[str, Any]) -> None:
                if account_id not in self._connections:
                    return
                emit_event(WS_EVENT_METHOD, event)
                # 暂时关闭买家自动回复入口；恢复时取消下面注释即可
