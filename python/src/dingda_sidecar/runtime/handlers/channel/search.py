"""Sidecar handler：``POST /v1/channel/search`` — 渠道关键词搜索（1688 / 闲鱼 / 小红书）。

按平台选择 fetch 实现，统一返回商品/offer 列表给 Rust。闲鱼搜索走
``crawlers.goofish``（系统 Camoufox + goofish_cli DOM）；小红书走
``crawlers.xiaohongshu``（vendored xhs-cli）。见 VENDOR.md。"""
...
from dingda_sidecar.crawlers.alibaba import fetch_search as fetch_ali1688_search
...
from dingda_sidecar.crawlers.goofish import fetch_search as fetch_goofish_search
from dingda_sidecar.crawlers.xiaohongshu import fetch_search as fetch_xiaohongshu_search

logger = logging.getLogger("dingda.sidecar.channel.search")

_SEARCH_FETCHERS = {
    "ali1688": fetch_ali1688_search,
    "xianyu": fetch_goofish_search,
    "xiaohongshu": fetch_xiaohongshu_search,
}
