...
比价数据流（先调研，再计划，再爬虫核验）::
    ...
    query ──────► 读（多查询）      读                 读
         ...
         按 keywords 抓闲鱼/1688 → xianyu_items / alibaba_items
              │
         normalize → match → analyze（对照调研核验）
              │                    │
              │         不足时 keyword_refine → 再 crawl（最多 3 轮）
              └────────────────────┴──► finalize → reply
"""
...
class GraphState(TypedDict, total=False):
    ...
    keywords_used: list[str]  # 已爬过的关键词（含各轮补充）
    crawl_round: int  # 爬取轮次；keyword_refine 递增
    continue_crawl: bool  # False 时 analyze 后不再回环
