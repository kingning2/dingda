...
由程序直接调用（``runtimes`` / IPC），不是 AI 可见 skill。
网页等公用工具见 ``tools.registry``（如 ``web_fetch`` / ``web_scrape``）。
...
节点顺序（每步会 ``ctx.step`` → 日志 + 可选回调）::
    ...
    web_research → article_analyze → planner → crawl
         → normalize → match → analyze ⇄ keyword_refine → crawl（不足时循环，最多 3 轮）
         → finalize → END
...
from dingda_sidecar.agent.graph.crawl_loop import route_after_analyze
...
def _compile_price_compare_graph(ctx: GraphContext) -> Any:
    ...
    graph.add_node(
        "keyword_refine",
        bind_context(ctx, keyword_refine_node, step="keyword_refine"),
    )  # 数据不足时 AI 补充关键词
    graph.add_node(
        "finalize",
...
def _route_after_analyze(state: GraphState) -> Literal["refine", "finalize"]:
    return route_after_analyze(state)


def _route_after_refine(state: GraphState) -> Literal["crawl", "finalize"]:
    if state.get("continue_crawl"):
        return "crawl"
    return "finalize"
