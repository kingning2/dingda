...
from runtimes.langgraph.errors import ErrorKind, GraphNodeError, classify_exception
from runtimes.langgraph.run_control import (
    GraphRun,
    NodeModel,
    StepRecord,
    deep_copy_state,
...
from skills.market_research.workflow import PRICE_COMPARE_STEPS

logger = logging.getLogger("dingda.graph.step_runner")

AI_NODES = frozenset({"web_research", "article_analyze", "planner", "analyze", "finalize"})
...
def _ctx_for_node(run: GraphRun, node: str) -> GraphContext:
    ...
    return GraphContext(config, steps=run.steps_order)
